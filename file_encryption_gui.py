...
def load_key() -> bytes | None:
    """Load the Fernet key from secret.key file."""
    try:
...
def encrypt_file() -> None:
    """Encrypt a selected file using the loaded key."""
    key = load_key()
    if not key:
        return
...
def decrypt_file() -> None:
    """Decrypt a selected file using the loaded key."""
    key = load_key()
    if not key:
        return
...
def run_gui() -> None:
    """Run the main GUI window for the File Encryption Tool."""
    root = Tk()
